...
<input type="number" min="1" max="20" name="prosmotr1" value="<?=htmlspecialchars($prosmotr1[1]??'');?>" required placeholder="3"/> и до:
<input type="number" min="1" max="20" name="prosmotr2" value="<?=htmlspecialchars($prosmotr2[1]??'');?>" required placeholder="5"/> шт
...
<input type="number" min="1" max="900" name="time1" value="<?=htmlspecialchars($time1[1]??'');?>" required placeholder="25"/> и до:
<input type="number" min="1" max="900" name="time2" value="<?=htmlspecialchars($time2[1]??'');?>" required placeholder="30"/> секунд
...
<font color="red">Внимательно ознакомьтесь с данным пунктом, его включение влияет на настройку</font><br /><br /><br />



<b>Связывать Referer и точки входа</b>:
<input name="refercheks" type="radio" id="refercheks-1" required  value="0" <?php if (($refercheks[1]??'') != '1') {echo 'checked';}?>>
<label for="refercheks-1"> отключить</label>
<input name="refercheks" type="radio" id="refercheks-2"  value="1" <?php if (($refercheks[1]??'') == '1') {echo 'checked';}?>>
...
<input type="radio" name="hostvar" id="host-1" value="0"  <?php if (($hostvar[1]??'') != '1') {echo 'checked';}?> required> -
<label for="host-1">Автоматически определять маску текущего сайта</label><br />
<input type="radio" name="hostvar" id="host-2" value="1" <?php if (($hostvar[1]??'') == '1') {echo 'checked';}?>> -
<label for="host-2">Использовать собственную маску</label>
<input type="text" name="host" id="host" placeholder="пример http://site.com"
<?php if (($hostvar[1]??'') == '1') {echo "value='".htmlspecialchars($host[1]??'')."'";}?> >
